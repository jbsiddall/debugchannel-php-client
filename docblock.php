...
function format_method( $method ) {
    ...
    $refl = $method->getReflectionObject();
    $docblock = new DocBlock((string)$refl->getDocComment());
...
HTML
    ...
    $annotationTypes = ["param", "return"];

    foreach( $annotationTypes as $type ) {
        $annotations = $method->getAnnotations(array($type));
        if( $annotations ) {
            foreach( $annotations as $annotation ) {
                print_r($annotation->getValue());
            }
        }
    }
